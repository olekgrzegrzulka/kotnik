...
glm::vec<2, float> accumulated_scroll_next{};
glm::vec<2, float> accumulated_scroll{};
} // namespace detail
...
void glfw_scroll_button_callback(GLFWwindow*, double xoffset, double yoffset) {
  detail::accumulated_scroll_next += glm::vec2{xoffset, yoffset};
}
...
void update() {
  if (!detail::glfw_window) {
    debug_error("Input not initialized");
  }
  double x, y;
  ...
  detail::accumulated_scroll_next = glm::vec2();
