...
void config_load_from_file(std::string file) {
  ...
  auto insert_ = [](std::string key, std::string value_str) {
    try {
      if (value_str.find('.') != std::string::npos) {
        float value = std::stof(value_str, nullptr);
        config_set_float(key, value);
      } else {
        i32 value = std::stoi(value_str, nullptr);
        config_set_i32(key, value);
      }
    } catch (std::exception) {
      config_set_string(key, value_str);
    }
  };
