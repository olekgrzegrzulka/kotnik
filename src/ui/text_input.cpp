...
TextInput::TextInput(UI& ui_) : Sprite(ui_), label(add_child<Label>()), caret(add_child<Sprite>()) {
  ...
  label.set_pos({2, 0});
  caret.set_screen_anchor(Anchor::CENTER_LEFT);
...
void TextInput::update() {
  ...
  bool rmb_just_pressed = Input::mouse_just_pressed(Input::Mouse::MOUSE_BUTTON_RIGHT);
  bool mmb_just_pressed = Input::mouse_just_pressed(Input::Mouse::MOUSE_BUTTON_MIDDLE);
  ...
  if (!focused) {
    set_texture("text_input_idle");
  } else {
    set_texture("text_input_focused");
  }
  ...
  if (focused) {
    ...
    caret.set_pos(glm::vec<2, i32>(label.get_width() + label.get_x() + 1, 0));
    caret_blink_clock -= 1;

    if (caret_blink_clock <= 0) {
      caret.set_process(!caret.get_process());
      caret_blink_clock = caret_blink_time;
    }

  } else {
    caret_blink_clock = 0;
    caret.set_process(false);
  }
}
