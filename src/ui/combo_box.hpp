...
public:
  ComboBoxItem(UI& ui_) : Button(ui_), label(add_child<Label>()) {
    set_screen_anchor(Anchor::TOP_CENTER);
    ...
    label.set_screen_anchor(Anchor::CENTER_LEFT);
...
public:
  ComboBox(UI& ui_) : Sprite(ui_), button(add_child<Button>()), button_icon(add_child<Sprite>()), dropdown_bg(add_child<Sprite>()), label_item(add_child<Label>()) {
    set_texture("combo_box");
    set_nine_slice_margin(5);

    button.set_screen_anchor(Anchor::CENTER_RIGHT);
    ...
    button_icon.set_screen_anchor(Anchor::CENTER_RIGHT);
    button_icon.set_anchor(Anchor::CENTER_RIGHT);
    button_icon.set_texture("combo_box_button_expand");
    button_icon.set_size(32, 32);

    dropdown_bg.set_texture("panel_rectangular");
    dropdown_bg.set_nine_slice_margin(3);
    dropdown_bg.set_screen_anchor(Anchor::BOTTOM_CENTER);
    dropdown_bg.set_anchor(Anchor::TOP_CENTER);
    dropdown_bg.set_clip_children(true);

    label_item.set_screen_anchor(Anchor::CENTER_LEFT);
  ...
  void update() {
    ...
    target_scroll_progress += Input::get_mouse_scroll().y * -1.0;
    target_scroll_progress = std::clamp<float>(target_scroll_progress, 0.0f, std::max<i32>(0, item_labels.size() - item_widgets.size() + 1));
    scroll_progress = std::lerp(scroll_progress, target_scroll_progress, 0.2);
    ...
    dropdown_bg.set_process(dropdown_state != DropDownState::HIDDEN);
