...
bool World::generate_chunk_column(i32 chunk_x, i32 chunk_z) {
  if (chunks.contains({chunk_x, 0, chunk_z})) { return false; }
  if (chunks_awaiting_generation.contains({chunk_x, chunk_z})) { return false; }
  if (chunks_being_generated.contains({chunk_x, chunk_z})) { return false; }
  chunks_awaiting_generation.emplace(glm::vec<2, i32>{chunk_x, chunk_z});
  return true;
}
...
void World::update() {
  ...
  if (player && Input::key_just_pressed(Input::Key::KEY_1)) {
    auto player_chunk_pos = cube_to_local(player->world_pos).first;
    world_lighter.spread_light(player_chunk_pos.x, player_chunk_pos.z);
  }
  ...
  if (worldgen_enabled) {
    for (i32 x = -chunk_load_distance; x <= chunk_load_distance; x += 1) {
      for (i32 z = -chunk_load_distance; z <= chunk_load_distance; z += 1) {
        ChunkPos chunk_pos{player_chunk_pos.x + x, 0, player_chunk_pos.z + z};
        generate_chunk_column(player_chunk_pos.x + x, player_chunk_pos.z + z);
      }
    }
  }

  std::multimap<i32, glm::vec<2, i32>, std::greater<i32>> chunks_awaiting_generation_sorted;

  for (auto chunk_pos : chunks_awaiting_generation) {
    i32 distance_to_chunk = manhattan_distance(chunk_pos, player_chunk_pos_xz);
  ...
  for (auto&& worker : chunk_terrain_gen_workers) {
    for (auto& chunk_column : worker->collect_finished_chunks()) {
      ensure(chunk_column.size() == max_chunk_y - min_chunk_y + 1);
      glm::vec<2, i32> chunk_column_pos = {chunk_column[0]->position.x, chunk_column[0]->position.z};
      ensure(!chunks_awaiting_generation.contains(chunk_column_pos));
      ensure(chunks_being_generated.contains(chunk_column_pos));
      chunks_being_generated.erase(chunk_column_pos);
      for (auto&& chunk : chunk_column) {
        ChunkPos chunk_pos = chunk->position;
        ensure(chunk_pos.x == chunk_column_pos.x && chunk_pos.z == chunk_column_pos.y);
  ...
  for (auto& [chunk_pos, chunk] : chunks) {
    bool chunk_is_too_far = (std::abs(player_chunk_pos.x - chunk_pos.x) > chunk_load_distance + 1) || (std::abs(player_chunk_pos.z - chunk_pos.z) > chunk_load_distance + 1);
